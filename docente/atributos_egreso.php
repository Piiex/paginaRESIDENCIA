...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['upload'])) {
    ...
    if (move_uploaded_file($file_tmp, $upload_dir . basename($file_name))) {
        ...
        $query = "INSERT INTO atributos_egreso_documentos (user_id, file_name, file_type, comment) VALUES (?, ?, ?, ?)";
        ...
        header("Location: atributos_egreso.php");
        exit();
    } else {
...
if (isset($_POST['delete'])) {
    ...
    $query = "SELECT file_name FROM atributos_egreso_documentos WHERE id = ?";
    $stmt = $conn->prepare($query);
    ...
    if ($result->num_rows > 0) {
        ...
        $query = "DELETE FROM atributos_egreso_documentos WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $file_id);
        $stmt->execute();
        $stmt->close();
...
$query = "SELECT * FROM atributos_egreso_documentos WHERE user_id = ?";
$stmt = $conn->prepare($query);
...
<head>
    ...
    <title>Documentos de Atributos de Egreso</title>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
        <div class="container">
            <a class="navbar-brand" href="../dashboard.php">Atributos de Egreso</a>
    ...
    <div class="container main-content mt-4">
        <h1 class="mb-4">Gestión de Documentos de Atributos de Egreso</h1>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form action="atributos_egreso.php" method="POST" enctype="multipart/form-data">
                            <div class="mb-3">
                        ...
                        <?php if ($result->num_rows > 0): ?>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <div class="file-item">
                                    ...
                                    <form action="atributos_egreso.php" method="POST" class="d-inline">
